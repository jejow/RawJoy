...
name_to_slug = {
    ...
    'Chicken Pa\u0302te\u0301': 'chicken-pumpkin-pate', # Pate with comb accents
    'Chicken P\u00e2t\u00e9': 'chicken-pumpkin-pate', # Pate with standard accents
...
def update_variant_dict(variant_obj, default_handle=None):
    # Determine the product handle/slug from the variant_obj context if present
    ...
    # Determine the weight from title, public_title, name, option1, etc.
    weight = None
...
def traverse_and_update(obj, default_handle=None):
    ...
    if isinstance(obj, dict):
        # Determine product handle if this dict is a product
        current_handle = default_handle
        for key in ['handle', 'slug', 'url', 'id']:
            val = obj.get(key)
            if isinstance(val, (str, int)):
                # If key is id, check if it maps to slug (id can be string in some contexts)
                val_str = str(val)
                if get_base_price(val_str) is not None:
                    current_handle = val_str
                    break
        
        # Check if this dict has variants
        ...
        # If not a product container, but itself might be a variant dict
        if not has_variants_list:
            if update_variant_dict(obj, default_handle):
                updated = True
                
        # Traverse other fields just in case
...
# Test run on venison-peas-recipe
path = 'products/venison-peas-recipe/index.html'
default_handle = 'venison-peas-recipe'
...
def replace_variants_cache(match):
    ...
    try:
        ...
        if traverse_and_update(data, default_handle):
            print("Updated a variants-cache script block!")
            # We want to format the output with indentations
            return f'<script data-variants-cache="" type="application/json">\n      {json.dumps(data, indent=2)}\n    </script>'
    except Exception as e:
        print(f"Error parsing cache block: {e}")
...
def replace_meta(match):
    ...
    try:
        data = json.loads(json_str)
        if traverse_and_update(data, default_handle):
            print("Updated meta block!")
            return f'var meta = {json.dumps(data, separators=(",", ":"))};'
    except Exception as e:
        print(f"Error parsing meta block: {e}")
    return match.group(0)
...
def replace_init_data(match):
    ...
    try:
        data = json.loads(json_str)
        if traverse_and_update(data, default_handle):
            print("Updated initData block!")
            return f'initData: {json.dumps(data, separators=(",", ":"))},'
    except Exception as e:
        print(f"Error parsing initData block: {e}")
    return match.group(0)
...
# This will catch other standalone JSON blocks
def replace_generic_json(match):
    tag_attrs = match.group(1)
    json_str = match.group(2)
    # Skip if it is a variants cache script (handled already)
    if 'data-variants-cache' in tag_attrs:
        return match.group(0)
    try:
        data = json.loads(json_str)
        if traverse_and_update(data, default_handle):
            print("Updated generic JSON block!")
...
# Let's save a temp file to compare
temp_path = 'products/venison-peas-recipe/index_temp_test.html'
with open(temp_path, 'w', encoding='utf-8') as f:
    f.write(new_content)

print(f"Test run completed. Temporary file written to {temp_path}")
